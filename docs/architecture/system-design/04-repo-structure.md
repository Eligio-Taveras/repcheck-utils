...
graph TB
    ...
    %% Cross-repo dependencies (published artifacts)
    %% Pipeline-operational dependencies (event schemas, job metadata, pub/sub)
    DI_SHARED --> PM_MODELS
    DI_SHARED --> SM_MODELS
    DI_BILLS --> DI_SHARED
    DI_VOTES --> DI_SHARED
    ...
    %% Domain data dependencies (legislative, user, LLM output types)
    PEB --> SM_MODELS
    PEU --> SM_MODELS
...
- **`repcheck-shared-models`** and **`repcheck-pipeline-models`** are published as versioned artifacts (e.g., to GitHub Packages or GCS Maven repo)
...
- Pipeline repos (`repcheck-data-ingestion`, `repcheck-llm-analysis`, `repcheck-scoring-engine`) depend on **both** `repcheck-shared-models` (domain types) and `repcheck-pipeline-models` (operational types)
- Non-pipeline repos (`repcheck-prompt-engine-bills`, `repcheck-prompt-engine-users`, `repcheck-api-server`) depend only on `repcheck-shared-models`
- `repcheck-llm-analysis` depends on `repcheck-prompt-engine-bills` as a published artifact
...
- Artifact publishing is automated via GitHub Actions on tagged releases
