...
**Pattern**: Process items as a stream. Each item is processed independently — failures are persisted immediately as `ProcessingResult` records, then the item is released from memory. An aggregator summarizes results after the stream completes.
...
- Each `ProcessingResult` is written to AlloyDB (`processing_results` table, keyed by `run_id`/`entity_id`) immediately
- After writing, the item and its result are released — no references retained
- The aggregator reads persisted results after stream completion to build the summary
- `PipelineRunSummary` is written to AlloyDB `pipeline_runs` table
...
**Pattern**: FS2 streams for memory-efficient processing. Individual items are processed and released — no accumulation.
...
- Use `.compile.drain` when you don't need to collect results (they're persisted per-item)
