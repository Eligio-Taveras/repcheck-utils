// =============================================================================
// RepCheck Skeleton: Pipeline Application Entry Point
// Repo: Each pipeline application (bill-ingestion, llm-analysis, scoring, etc.)
// =============================================================================
//
// PURPOSE: Cloud Run Job entry point pattern. Extends IOApp, loads snapshots
// from GCS, processes items via FS2 streaming with fail-and-continue, writes
// PipelineRunSummary, and updates run status in DB.
//
...
import repcheck.config.ConfigLoader
import repcheck.pipeline.models.gcs.GcsClient
import repcheck.pipeline.models.Tables
import doobie.Transactor
import repcheck.pipeline.models.pubsub.{PipelineEvent, PubSubPublisher}
import repcheck.pipeline.snapshot.{SnapshotManifest, SnapshotService}
import repcheck.pipeline.streaming.{PipelineRunner, PipelineRunSummary}
...
/** Base pattern for all pipeline apps. Copy this and fill in:
  * 1. Your AppConfig type
  * 2. Your input DTO type
  * 3. Your per-item processing function
  * 4. Your downstream PipelineEvent payload
  */
trait PipelineApp {
